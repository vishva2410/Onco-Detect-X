...
class PerceptionService:
    def predict(self, image_data: bytes, cancer_type: str) -> PerceptionOutput:
        # Create a deterministic random instance based on image content
        # This ensures that the same image always yields the same "prediction",
        # allowing downstream services (like CognitiveService) to cache results effectively.
        seed_val = zlib.adler32(image_data)
